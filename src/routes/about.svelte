<script context="module">
  ...
  // if (res.ok) return { props: { evalTemplates: await res.json() } };
  // return {
  //   status: res.status,
  //   error: new Error(),
  // };
  ...
  export const load = async ({ fetch }) => {
    const res = await fetch("/api/_getTemplates.js");

    if (res.ok) {
      const evalTemplates = await res.json();
...
<script>
  export let evalTemplates;

  console.log("Users: ", evalTemplates);
</script>
